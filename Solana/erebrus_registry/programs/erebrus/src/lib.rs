...
pub mod erebrus_registry {
    ...
    pub fn initialize(ctx: Context<Initialize>) -> Result<()> {
        ...
        state.current_wifi_node = 0;
        state.current_vpn_node = 0;
    ...
    pub fn register_vpn_node(
        ...
        let state = &mut ctx.accounts.state;
        let node_id = state.current_vpn_node;
        ...
        vpn_node.status = 0;
        vpn_node.node_id = node_id;
        vpn_node.can_close = false;

        state.current_vpn_node = state.current_vpn_node.checked_add(1).unwrap();
